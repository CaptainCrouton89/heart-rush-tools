...
const TALENTS_DIR = path.join("heart_rush", "talents", "noncombat_talents");

async function fixSubheaderInFile(filePath: string) {
  const content = await fs.readFile(filePath, "utf-8");
  const lines = content.split(/\r?\n/);

  // Find the first header (##, ###, or #)
  let headerIdx = lines.findIndex((l) => /^(#|##|###) /.test(l));
...
async function main() {
  const files = (await fs.readdir(TALENTS_DIR)).filter((f) =>
    f.endsWith(".md")
  );
  let changed = 0;
  ...
  console.log(
    `\n✅ Fixed subheader line position in ${changed} noncombat talent files.`
  );
}
